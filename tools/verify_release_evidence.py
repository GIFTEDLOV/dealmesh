...
def verify() -> list[str]:
    ...
    require(create.get("attemptCount") == 4 and len(attempts) == 4, "create attempt count must be four")
    ...
    final_attempt = attempts[3]
    require(final_attempt.get("sequence") == 4, "final create attempt sequence mismatch")
    require(final_attempt.get("returnedHash") == "0xb90302aae0826778cb05bd503ce3ebc61a40b812f8b8ccf89bdcd0dabf349a0f", "final create hash mismatch")
    require(final_attempt.get("status") == "ACCEPTED", "final create status mismatch")
    require(final_attempt.get("executionResult") == "FAILED", "final create execution mismatch")
    require(final_attempt.get("rpcCode") is None and final_attempt.get("rpcMessage") is None, "final create attempt incorrectly classified as pre-hash rejection")
    ...
    require(reconciliation.get("transactionHash") == final_attempt.get("returnedHash"), "same-hash reconciliation mismatch")
    require(reconciliation.get("status") == "ACCEPTED" and reconciliation.get("executionResult") == "FAILED", "same-hash terminal evidence mismatch")
    require(reconciliation.get("traceError") == "CANONICALIZATION_FAILED", "create failure trace mismatch")
    require(manifest.get("lifecycleStatus") == "CREATE_DEAL_FAILED", "manifest lifecycle status mismatch")
    require(manifest.get("publicFrontend", {}).get("status") == "NOT_DEPLOYED", "manifest frontend status mismatch")
    writes = manifest.get("writes", [])
    require(len(writes) == 2 and writes[0].get("operation") == "deploy" and writes[1].get("operation") == "create_deal", "manifest write records mismatch")
    require(manifest.get("transactionHashes") == [EXPECTED_DEPLOYMENT, final_attempt.get("returnedHash")], "transaction hash list mismatch")
    ...
    require(final_proof_attempt.get("returnedHash") == final_attempt.get("returnedHash"), "proof final create hash mismatch")
    ...
    require(proof_absence.get("latestNonce") == 204 and proof_absence.get("pendingNonce") == 204, "final proof nonce evidence mismatch")
    ...
    require(lifecycle.get("status") == "CREATE_DEAL_FAILED", "final proof lifecycle status mismatch")
    require(lifecycle.get("createDealHash") == final_attempt.get("returnedHash"), "final proof create hash mismatch")
    ...
    for path, text in (("README.md", readme), ("SUBMISSION.md", submission)):
        ...
        lower = text.lower()
        require("lifecycle has not started" in lower or "lifecycle remains unstarted" in lower, f"{path} overclaims live lifecycle")
